...
def applyGamma(image_uint8, gamma=2.2):
    image_float64 = image_uint8.astype(np.float64)
    image_applyed_gamma = (((image_float64/255.0)**(1/gamma)) * 255).astype(np.uint8)
    return image_applyed_gamma 
...
def main():
    ...
    img_AoLP_col = pa.applyLightColorToAoLP(img_AoLP)
    ...
    #Export images
    cv2.imwrite(name+"_intensity"+ext, applyGamma(imnormarize(img_intensity, max_val, max_val, dtype)))
    ...
    cv2.imwrite(name+"_AoLP"  +".png", img_AoLP_col)
    cv2.imwrite(name+"_specular"+".png", imnormarize(img_specular, max_val, max_val, dtype))
    cv2.imwrite(name+"_diffuse"+".png", imnormarize(img_diffuse, max_val, max_val, dtype))
