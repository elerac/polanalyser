...
def __demosacing_mono(img_mpfa):
    if img_mpfa.dtype==np.uint8 or img_mpfa.dtype==np.uint16:
        return __demosaicing_mono_uint(img_mpfa)
    else:
        return __demosaicing_mono_float(img_mpfa)
...
def __demosaicing_color(img_cpfa):
    ...
    for i, img_mpfa in enumerate(cv2.split(img_mpfa_bgr)):
        img_demosaiced = demosaicing(img_mpfa, "Polar-Mono")
        img_bgr_polarization[..., i, :] = img_demosaiced

    return img_bgr_polarization

def demosaicing(img_raw, code="Polar-Mono"):
    """
    ...
    code : str
        "Polar-Mono" or "Polar-RGB"
    """
    if   code == "Polar-Mono":
        return __demosacing_mono(img_raw)
    elif code == "Polar-RGB":
